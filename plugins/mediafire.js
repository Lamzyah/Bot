...
let handler = async (m, { conn, args, isPrems, isOwner, usedPrefix, command }) => {
    ...
    m.reply(wait);
    let res = await fetch(`${webapi}api/downloader/mediafire?url=${args[0]}&apikey=${apichan}`);
...
`;
    m.reply(cap);
    await conn.sendMessage(m.chat, { document: {url: v.url}, mimetype: 'document', fileName: `${v.title}`}, {quoted: m})
}
